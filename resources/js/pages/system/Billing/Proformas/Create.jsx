...
export default function CreateProforma({ statuses, nextNumber, clients, trips }) {
    ...
    return (
        <DashboardLayout title="New Proforma Invoice">
            ...
            <Stack gap={2} mb="xl">
                <Text fw={800} size="xl" style={{ color: textPri }}>New Proforma Invoice</Text>
                <Text size="sm" style={{ color: textSec }}>Auto-number: <b style={{ color: '#A78BFA' }}>{nextNumber}</b></Text>
            </Stack>
            <BillingForm data={data} setData={setData} errors={errors} statuses={statuses} clients={clients} trips={trips} processing={processing} onSubmit={submit} backHref="/system/billing/proformas" submitLabel="Create Proforma" documentType="proforma" />
        </DashboardLayout>
    );
}
