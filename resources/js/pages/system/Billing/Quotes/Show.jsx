...
export default function ShowQuote({ quote, statuses, company }) {
    ...
    const statusInfo = statuses[quote.status] ?? { label: quote.status, color: '#94A3B8' };
        ...
        <DashboardLayout title={`Quote · ${quote.document_number}`}>
                    ...
                    <Group justify="space-between" align="center" wrap="wrap" gap="md" style={{ position: 'relative', zIndex: 1 }}>
                        <Stack gap={6}>
                            <Group gap={10} align="center">
                                ...
                                <Stack gap={2}>
                                    <Group gap={10} align="center">
                                        <Text fw={900} size="xl" c="white" style={{ letterSpacing: 0.5 }}>{quote.document_number}</Text>
                                    ...
                                    <Text size="sm" style={{ color: 'rgba(255,255,255,0.8)' }}>
                                        {quote.client?.name}{quote.client?.company_name ? ` — ${quote.client.company_name}` : ''}
                                    </Text>
                                </Stack>
                            </Group>
                        </Stack>
                        ...
                        <Group gap={8} wrap="wrap">
                            {can('billing_quotes.edit') && ['draft', 'sent', 'accepted'].includes(quote.status) && (
                                <motion.div whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}>
                                    <Box component="button" onClick={convertToProforma}
                            ...
                            {can('billing_quotes.edit') && (
                                <Box component={Link} href={`/system/billing/quotes/${quote.id}/edit`}
                ...
                <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}>
                    <Box mb={16} style={{ background: isDark ? 'rgba(59,130,246,0.08)' : '#EFF6FF', border: `1px solid ${isDark ? 'rgba(59,130,246,0.2)' : '#BFDBFE'}`, borderRadius: 10, padding: '10px 16px' }}>
                        <Text size="sm" style={{ color: '#3B82F6' }}>
                            Converted from{' '}
                            <Box component={Link} href={`/system/billing/quotes/${quote.converted_from.id}`} style={{ color: '#3B82F6', fontWeight: 700, textDecoration: 'none' }}>{quote.converted_from.document_number}</Box>
                        </Text>
                    </Box>
                </motion.div>
            )}
            ...
            <SimpleGrid cols={{ base: 1, sm: 2 }} spacing="md" mb="md">
                <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
                    <SectionCard title="Quote Details" icon="📋" isDark={isDark} accent={['#1565C0', '#2196F3']}>
                        <InfoRow icon="📅" label="Issue Date"   value={fmtDate(quote.issue_date)}   isDark={isDark} />
                        <InfoRow icon="⏳" label="Valid Until"  value={fmtDate(quote.valid_until)}  isDark={isDark} />
                        <InfoRow icon="📆" label="Due Date"     value={fmtDate(quote.due_date)}     isDark={isDark} />
                        {quote.trip && (
                            <Box style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 0', borderBottom: `1px solid ${divider}`, gap: 12 }}>
                                <Group gap={8}>
                                    <Text size="sm">🚛</Text>
                                ...
                                <Box component={Link} href={`/system/trips/${quote.trip.id}`} style={{ color: '#EA580C', fontSize: 13, fontWeight: 700, textDecoration: 'none' }}>{quote.trip.trip_number}</Box>
                            </Box>
                        )}
                    </SectionCard>
                </motion.div>
                ...
                <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
                    <SectionCard title="Bill To" icon="🏢" isDark={isDark} accent={['#0E4FA0', '#3B82F6']}>
                        <Box style={{ paddingTop: 8 }}>
                            <Text fw={800} size="sm" style={{ color: textPri, marginBottom: 4 }}>{quote.client?.name}</Text>
                            {quote.client?.company_name && <Text size="sm" style={{ color: textSec, marginBottom: 2 }}>{quote.client.company_name}</Text>}
                            {quote.client?.address && <Text size="sm" style={{ color: textSec, marginBottom: 2 }}>{quote.client.address}</Text>}
                            {quote.client?.tin_number && (
                                <Box style={{ display: 'inline-flex', background: isDark ? 'rgba(255,255,255,0.06)' : '#F1F5F9', borderRadius: 6, padding: '2px 8px', marginTop: 4 }}>
                                    <Text size="xs" fw={600} style={{ color: textMut, fontFamily: 'monospace' }}>TIN: {quote.client.tin_number}</Text>
                                </Box>
                            )}
                        </Box>
                    </SectionCard>
                ...
                <Box mb="md" style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: 16, overflow: 'hidden', boxShadow: cardShadow }}>
                    ...
                    <Box style={{ padding: '16px 20px', borderTop: `1px solid ${divider}`, maxWidth: 360, marginLeft: 'auto' }}>
                        <Group justify="space-between" mb={6}>
                            ...
                            <Text size="sm" style={{ color: textPri }}>{cur} {fmt(quote.subtotal)}</Text>
                        </Group>
                        {Number(quote.discount_amount) > 0 && (
                            <Group justify="space-between" mb={6}>
                                <Text size="sm" style={{ color: textSec }}>Discount</Text>
                                <Text size="sm" style={{ color: '#EF4444' }}>− {cur} {fmt(quote.discount_amount)}</Text>
                            </Group>
                        )}
                        {Number(quote.tax_rate) > 0 && (
                            <Group justify="space-between" mb={6}>
                                <Text size="sm" style={{ color: textSec }}>VAT ({quote.tax_rate}%)</Text>
                                <Text size="sm" style={{ color: textPri }}>{cur} {fmt(quote.tax_amount)}</Text>
                            </Group>
                        )}
                        <Box style={{ borderTop: `2px solid ${isDark ? 'rgba(255,255,255,0.1)' : '#E2E8F0'}`, paddingTop: 10, marginTop: 6 }}>
                            <Group justify="space-between">
                                ...
                                <Text fw={900} size="lg" style={{ color: '#22C55E' }}>{cur} {fmt(quote.total)}</Text>
                            </Group>
                        </Box>
                    </Box>
                </Box>
            ...
            {(quote.notes || quote.terms_conditions) && (
                <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}>
                    <SimpleGrid cols={{ base: 1, sm: 2 }} spacing="md" mb="md">
                        {quote.notes && (
                            <SectionCard title="Notes" icon="📝" isDark={isDark} accent={['#059669', '#10B981']}>
                                <Text size="sm" style={{ color: textSec, whiteSpace: 'pre-wrap', paddingTop: 8 }}>{quote.notes}</Text>
                            </SectionCard>
                        )}
                        {quote.terms_conditions && (
                            <SectionCard title="Terms & Conditions" icon="📋" isDark={isDark} accent={['#7C3AED', '#A78BFA']}>
                                <Text size="sm" style={{ color: textSec, whiteSpace: 'pre-wrap', paddingTop: 8 }}>{quote.terms_conditions}</Text>
                            </SectionCard>
                        )}
                    </SimpleGrid>
                </motion.div>
            ...
            <Box mt="xl">
                <Box component={Link} href="/system/billing/quotes"
