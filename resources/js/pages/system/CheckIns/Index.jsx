        ...
        <DashboardLayout title="Driver Check-Ins">
                ...
                <Box mb={24} style={{
                    ...
                    <Group justify="space-between" align="center" wrap="wrap" gap="md" style={{ position: 'relative', zIndex: 1 }}>
                        <Group gap={10}>
                            ...
                            <Stack gap={1}>
                                <Text fw={900} size="lg" c="white">Driver Check-Ins</Text>
                                <Text size="xs" style={{ color: 'rgba(255,255,255,0.7)' }}>3-hour interval status reports from drivers on active trips</Text>
                            </Stack>
                        </Group>
                    </Group>
                </Box>
            ...
            <Box style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: 16, overflow: 'hidden', boxShadow: cardShadow }}>
                ...
                <Box style={{ padding: '14px 20px', borderBottom: `1px solid ${divider}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    ...
                    <Text size="xs" style={{ color: textMut }}>{checkIns.total ?? checkIns.data.length} total</Text>
                </Box>

                {/* Head */}
                <Box style={{ display: 'grid', gridTemplateColumns: cols, background: headBg, borderBottom: `1px solid ${divider}`, padding: '10px 20px', gap: 0 }}>
                    {['Checked in', 'Driver / Trip', 'Status', 'Vehicle', 'Location', 'Notes'].map(h => (
                ...
                {checkIns.data.length === 0 ? (
                    <Box style={{ textAlign: 'center', padding: '72px 0' }}>
                        ...
                        <Text fw={800} size="md" style={{ color: textPri, marginBottom: 6 }}>No check-ins yet</Text>
                        <Text size="sm" style={{ color: textMut }}>Check-ins from active drivers will appear here</Text>
                    </Box>
                ) : (
                    checkIns.data.map((c, idx) => {
                        ...
                        return (
                            <motion.div key={c.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.03 }}>
                                <Box
                                    ...
                                    {/* Time */}
                                    <Text size="xs" fw={600} style={{ color: textSec }}>{new Date(c.checked_in_at).toLocaleString()}</Text>

                                    {/* Driver / Trip */}
                                    <Stack gap={2}>
                                        <Text size="sm" fw={600} style={{ color: textPri }}>{c.driver?.name ?? '—'}</Text>
                                        {c.trip && (
                                            <Box style={{ display: 'inline-flex', background: isDark ? 'rgba(234,88,12,0.12)' : '#FFF7F0', border: '1px solid rgba(234,88,12,0.25)', borderRadius: 6, padding: '2px 8px', width: 'fit-content' }}>
                                                <Text size="xs" fw={700} style={{ color: '#EA580C', fontFamily: 'monospace' }}>{c.trip.trip_number}</Text>
                                            </Box>
                                        )}
                                        {!c.trip && <Text size="xs" style={{ color: textMut }}>—</Text>}
                                    </Stack>
                                    ...
                                    {/* Vehicle */}
                                    {c.vehicle?.plate
                                        ? <Box style={{ display: 'inline-flex', background: isDark ? 'rgba(96,165,250,0.12)' : '#EFF6FF', border: '1px solid rgba(96,165,250,0.25)', borderRadius: 8, padding: '3px 8px', width: 'fit-content' }}>
                                            <Text size="xs" fw={700} style={{ color: '#60A5FA', fontFamily: 'monospace' }}>{c.vehicle.plate}</Text>
                                          </Box>
                                    ...
                                    {/* Notes */}
                                    <Text size="xs" style={{ color: textMut, whiteSpace: 'pre-wrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{c.notes || '—'}</Text>
                                </Box>
                            </motion.div>
                        );
                    })
                ...
                {checkIns.data.length > 0 && (
                    <Box style={{ padding: '10px 20px', borderTop: `1px solid ${divider}`, background: headBg }}>
                        <Text size="xs" style={{ color: textMut }}>{checkIns.total ?? checkIns.data.length} total check-in{(checkIns.total ?? checkIns.data.length) !== 1 ? 's' : ''}</Text>
                    </Box>
                )}
            </Box>
            ...
            {checkIns.last_page > 1 && (
                <Group justify="center" mt="lg">
                    <Pagination
                        ...
                        onChange={p => router.get('/system/check-ins', { ...filters, page: p })}
                        size="sm"
